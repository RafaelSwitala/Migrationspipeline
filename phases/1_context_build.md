...
Erstelle ür `<FEATURE_NAME>` einen vollsändigen, belegten Feature-Kontext aus iOS und Android, sodass Phase 2 bis 5 kein neues fachliches Legacy-Discovery mehr brauchen.
...
Die Chat-Eingabe selbst ist nur der Ausöser, kein fachlicher Inhalt außer `FEATURE_NAME`.
...
| P1-A13 | `13_test_definition.md` | testbare Behaviors, geplante `LT-*` Tests, Edge Cases, Coverage-Ziele, nicht testbare Punkte | jeder wichtige `BEH-*`/`STATE-*`/`ERRPATH-*` ist getestet oder begründet ausgeschlossen |
| P1-A14 | `14_migration_mapping.md` | `MAP-*` für RN-Ziele, Services, Storage, API, State, Divergenzen, Dependencies | jedes relevante Code Fact hat Mapping oder Ausschlussgrund |
| P1-A15 | `15_execution_contract.md` | Testframeworks, Mocks, Commands, spätere Phasenregeln, bekannte Build-/Test-Hinweise | Phase 2 bis 5 können ohne neues fachliches Discovery starten |
...
Keine Codeänderungen in Legacy- oder RN-Repositories.
...
1. Run initialisieren.
   ...
   - Füll `run_metadata.md` mit Agent, Modell, Prompt, Zeit und Artefaktwurzel.
...
3. Relevanz entscheiden.
   ...
   - Jede relevante Datei erhält `IOS-FILE-*` oder `AND-FILE-*`.
...
4. Code Facts extrahieren.
   ...
   - Widersprüche zwischen iOS und Android nicht auflösen, sondern als Divergenz dokumentieren.
...
7. Execution Contract erstellen.
   - Dokumentiere, welche Informationen Phase 2 bis 5 verwenden müssen.
   - Notiere bekannte Testcommands oder `UNKNOWN` mit Grund.

8. Traceability konsolidieren.
   - Verknüpfe jede Source-ID mit Tests, Mappings und RN-Zielen.
   - Markiere Gaps als `BLOCKED`, wenn sie spätere Phasen verhindern.

9. Self-Validation.
   - Führe alle Validierungsregeln aus dieser Phase durch.
...
| VAL-P1-02 | Code Facts vollständig oder `N/A` begründet | ERR-P1-02 |
| VAL-P1-03 | Spätere Phasen können ohne fachliches Rediscovery arbeiten | ERR-P1-02 |
...
| Fehler | Behandlung |
|---|---|
| ERR-P1-01 | STOP, Feature-Scope oder Suchbegriffe klären |
| ERR-P1-02 | STOP, fehlendes Artefakt oder fehlende IDs ergänzen |
