...
Erzeuge und fuehre fuer `<FEATURE_NAME>` Unit Tests in den bestehenden iOS- und Android-Codebasen aus. Die Tests muessen aus Phase 1 ableitbar sein und eine moeglichst hohe sinnvolle Coverage erreichen.
...
Legacy-Code darf in Phase 2 nur fuer Testplatzierung, Imports, Buildmechanik und Fehlersuche beim Testlauf geoeffnet werden. Neue fachliche Ableitungen sind verboten.
...
|---|---|---|---|
| P2-A21 | `21_legacy_test_plan.md` | alle `LT-*` Tests aus Phase 1 mit Plattform, Given/When/Then, Quellen, Prioritaet | jeder Test referenziert Phase-1-IDs |
| P2-A22 | `22_legacy_test_implementation.md` | erzeugte/geaenderte Testdateien, Frameworks, Mocks, Test-ID zu Datei | jede Codeaenderung ist dokumentiert |
| P2-A23 | `23_legacy_test_results.md` | ausgefuehrte Commands, Testresultate, Failure Details, Coverage je Plattform | jeder valide Test hat `PASS`, `FAIL`, `SKIP`, `NOT_RUN` oder `INVALID` |
| P2-A24 | `24_legacy_test_quality.md` | Bewertung nach `TEST-006`, Coverage-Gaps, schwache Tests, Failure-Sensitivitaet | jeder Test ist qualitativ bewertet |
...
Keine produktiven Legacy-Implementierungen aendern, ausser eine minimale testtechnische Sichtbarkeit oder Testkonfiguration ist zwingend notwendig und dokumentiert.
...
4. Unit Tests implementieren.
   ...
   - Nutze Test Doubles/Mocks nur fuer externe Systeme.
   - Keine Tests, die nur Klassenexistenz oder Snapshots ohne Behavior pruefen.

5. Tests und Coverage ausfuehren.
   - Fuehre die passenden Android- und iOS-Unit-Testcommands aus.
   - Erfasse Pass/Fail/Skip/Invalid nach `OUT-004`.
   - Erfasse Coverage nach `OUT-005`, wenn verfuegbar.

6. Testqualitaet bewerten.
   - Pruefe jeden Test gegen `TEST-006`.
   - Markiere schwache Tests und erklaere, ob sie verbessert wurden oder bewusst bleiben.

7. Ergebnisse dokumentieren.
   - Fuell alle vier Phase-2-Artefakte.
...
| VAL-P2-03 | Alle validen Tests ausgefuehrt oder Ausfuehrungsfehler dokumentiert | ERR-P2-03 |
| VAL-P2-04 | Coverage erfasst oder `N/A` begruendet | ERR-P2-03 |
